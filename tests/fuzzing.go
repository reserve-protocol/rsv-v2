...
func TestFuzzing(t *testing.T) {
	suite.Run(t, new(FuzzingSuite))
}

type FuzzingSuite struct {
	TestSuite
...
var (
	// Compile-time check that FuzzingSuite implements the interfaces we think it does.
	// If it does not implement these interfaces, then the corresponding setup and teardown
	// functions will not actually run.
	_ suite.BeforeTest       = &FuzzingSuite{}
	_ suite.SetupAllSuite    = &FuzzingSuite{}
	_ suite.TearDownAllSuite = &FuzzingSuite{}
)

// SetupSuite runs once, before all of the tests in the suite.
func (s *FuzzingSuite) SetupSuite() {
	s.setup()
}

// TearDownSuite runs once, after all of the tests in the suite.
func (s *FuzzingSuite) TearDownSuite() {
	if coverageEnabled {
...
func (s *FuzzingSuite) BeforeTest(suiteName, testName string) {
	s.owner = s.account[0]
...
// func (s *FuzzingSuite) TestSetIssuancePaused() {
...
// func (s *FuzzingSuite) TestSetIssuancePausedIsProtected() {
...
// func (s *FuzzingSuite) TestSetEmergency() {
...
// func (s *FuzzingSuite) TestSetEmergencyIsProtected() {
...
// func (s *FuzzingSuite) TestSetOperator() {
...
// func (s *FuzzingSuite) TestSetOperatorIsProtected() {
...
// func (s *FuzzingSuite) TestSetSeigniorage() {
...
// func (s *FuzzingSuite) TestSetSeigniorageIsProtected() {
// 	seigniorage := bigInt(1)
...
// func (s *FuzzingSuite) TestSetDelay() {
...
// func (s *FuzzingSuite) TestSetDelayIsProtected() {
// 	delay := bigInt(1)
...
// func (s *FuzzingSuite) TestClearProposals() {
...
// func (s *FuzzingSuite) TestClearProposalsIsProtected() {
...
// func (s *FuzzingSuite) TestIssueIsProtected() {
// 	amount := bigInt(1)
...
// func (s *FuzzingSuite) TestIssueRequireStatements() {
// 	amount := bigInt(1)
...
// func (s *FuzzingSuite) TestRedeem() {
// 	// Issue.
// 	rsvAmount := shiftRight(1, 27) // 1 billion
...
// func (s *FuzzingSuite) TestRedeemIsProtected() {
// 	// Issue.
// 	rsvAmount := shiftRight(1, 27) // 1 billion
// 	s.requireTx(s.manager.Issue(signer(s.proposer), rsvAmount))
...
// func (s *FuzzingSuite) TestRedeemRequireStatements() {
// 	// Issue.
// 	rsvAmount := shiftRight(1, 27) // 1 billion
// 	s.requireTx(s.manager.Issue(signer(s.proposer), rsvAmount))
...
// func (s *FuzzingSuite) TestProposeWeightsFullUsecase() {
// 	// Issue a billion RSV.
...
// func (s *FuzzingSuite) TestProposeSwapFullUsecase() {
// 	// Issue a billion RSV.
// 	rsvToIssue := shiftRight(1, 27) // 1 billion
// 	s.requireTx(s.manager.Issue(signer(s.proposer), rsvToIssue))
// 	s.assertManagerCollateralized()
...
func (s *FuzzingSuite) changeBasketUsingWeightProposal(tokens []common.Address, weights []*big.Int) {
...
func (s *FuzzingSuite) changeBasketUsingSwapProposal(tokens []common.Address, amounts []*big.Int, toVault []bool) {
	// Propose the new basket.
...
func (s *FuzzingSuite) computeExpectedIssueAmounts(
...
func (s *FuzzingSuite) computeExpectedRedeemAmounts(rsvSupply *big.Int) []*big.Int {
	// Get current basket.
	basketAddress, err := s.manager.TrustedBasket(nil)
	s.Require().NoError(err)
	basket, err := abi.NewBasket(basketAddress, s.node)
...
func (s *FuzzingSuite) newWeights(
...
func (s *FuzzingSuite) fundAccountWithErc20sAndApprove(acc account, amounts []*big.Int) {
